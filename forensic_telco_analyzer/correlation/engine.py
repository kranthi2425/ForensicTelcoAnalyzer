...
class CorrelationEngine:
    ...
    def load_data(self, cdr_file=None, ipdr_file=None, tdr_file=None):
        ...
        if cdr_file and os.path.exists(cdr_file):
            ...
            logging.info(f"Loaded CDR data: {len(self.cdr_data)} records")
        ...
        if ipdr_file and os.path.exists(ipdr_file):
            ...
            logging.info(f"Loaded IPDR data: {len(self.ipdr_data)} records")
        ...
        if tdr_file and os.path.exists(tdr_file):
            ...
            logging.info(f"Loaded TDR data: {len(self.tdr_data)} records")
    ...
    def correlate_cdr_tdr(self, time_window_minutes=30):
        ...
        if self.cdr_data is None or self.tdr_data is None:
            logging.error("Both CDR and TDR data must be loaded for correlation.")
            return None

        logging.info("Correlating CDR and TDR data...")
        ...
        if result_df.empty:
            logging.warning("No correlations found between CDR and TDR data.")
            return None

        logging.info(f"Found {len(result_df)} correlations between calls and tower pings.")
    ...
    def correlate_ipdr_cdr(self, time_window_minutes=5):
        ...
        if self.ipdr_data is None or self.cdr_data is None:
            logging.error("Both IPDR and CDR data must be loaded for correlation.")
            return None

        logging.info("Correlating IPDR and CDR data...")
        ...
        result_df = pd.DataFrame(results)
        if result_df.empty:
            logging.warning("No correlations found between IPDR and CDR data.")
            return None

        logging.info(f"Found {len(result_df)} correlations between calls and IP traffic.")
    ...
    def correlate_all(self, time_window_minutes=30):
        ...
        if cdr_tdr_corr is not None and ipdr_cdr_corr is not None:
            ...
            if not merged.empty:
                logging.info(f"Found {len(merged)} comprehensive correlations across all datasets.")
                self.correlation_results['all'] = merged
                return merged

        logging.warning("No comprehensive correlations found across all datasets.")
        return None

    def correlate_osint_with_cdr(self, osint_file, cdr_file):
    ...
    def save_results(self, output_dir):
        ...
        for key, df in self.correlation_results.items():
            ...
            logging.info(f"Saved {key} correlation results to {file_path}.")
